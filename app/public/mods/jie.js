...
layui.define(['fly','laypage'], function(exports){
  ...
  form.on('select(column)', function(obj){
    ...
    if(value === '0'){
      ...
      layer.tips('只有系统管理员才能发公告，请确认是否有权限发布！', obj.othis, tips);
    }
  });
  ...
  //渲染分页
  if(typeof window.PageCount != 'undefined' && window.PageCount > 0 ){
    
    laypage.render({
      elem: 'page-box'
      ,count: window.PageCount
      ,curr:window.PageIndes
      ,limit: 20
    });

    $("#page-box a").on('click',function(){
      var str = $(this).attr('class') || 'no';
      if(str.indexOf('layui-disabled') != -1){
        return false;
      }
      var n = $(this).attr('data-page');
      var url = $('#page-box').attr('data-url');
